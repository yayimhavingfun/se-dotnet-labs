...
public class FileCopyCommand : ICommand
{
    ...
    public FileSystemResult Execute()
    {
        ...
        if (_context.CurrentFileSystem == null)
            return new FileSystemResult.OperationFailed("FileCopy", "File system is not available");

        if (string.IsNullOrWhiteSpace(_sourcePath))
            return new FileSystemResult.OperationFailed("FileCopy", "Source path is empty");

        if (string.IsNullOrWhiteSpace(_destinationPath))
            return new FileSystemResult.OperationFailed("FileCopy", "Destination path is empty");

        return _context.CurrentFileSystem.CopyFile(_sourcePath, _destinationPath);
    }
}